...
Se sabe que el **error cuadrático medio (ECM)** es $\text{ECM}(\hat{\theta}) = 8$, y se sabe que $P(\hat{\theta} \leq \theta) = 0.8413$. La fórmula del ECM de un estimador es $\text{ECM}(\hat{\theta}) = \text{Var}(\hat{\theta}) + \text{Sesgo}(\hat{\theta})^2$, donde $\text{Sesgo}(\hat{\theta}) = \mathbb{E}[\hat{\theta}] - \theta$
