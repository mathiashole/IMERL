...
A continuación, se calculará la información de Fisher para distintas distribuciones de probabilidad.

## Caso (a): Distribución Bernoulli $X \sim \text{Ber}(p)$

La función de probabilidad de una distribución Bernoulli es:
...
## Caso (b): Distribución Exponencial $X \sim \exp(\lambda)$
...
## Caso (c): Distribución Normal $X \sim N(\mu, 4)$
