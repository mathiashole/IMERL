...
Para encontrar la distribución de $Y$, utilizamos las propiedades de la normalidad, dado que $Y$ es una combinación lineal de las variables aleatorias normales.

Calcular la media de $Y$ sustituyendo con $\mu = (−1, 1, 0)$:
...
Calcular la varianza de $Y$ usando la matriz de covarianzas:

$$
\text{Var}(Y) = \text{Var}(X_1) + 2^2\text{Var}(X_2) + (-3)^2\text{Var}(X_3) + 2(1)(2)\text{Cov}(X_1, X_3) + 2(-3)(1)\text{Cov}(X_2, X_3)
$$

De la matriz de covarianzas $\Sigma$, extraemos los elementos necesarios:
...
Sustituyendo en la fórmula de varianza:

$$
\text{Var}(Y) = 1 + 2^2(3) + (-3)^2(2) + 2(1)(2)(1) + 2(-3)(1)(1)
$$

Calculamos:

$$
\text{Var}(Y) = 1 + 4(3) + 9(2) + 2(1)(2) + 2(-3)(1)
$$
